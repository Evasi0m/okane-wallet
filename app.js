...
applyCustomIcons();
function applyIndexSvgOverrides(){var ci=(function(){try{return JSON.parse(localStorage.getItem('okane_v3')||'{}').customIcons||{}}catch(e){return{}}})();var nodes=document.querySelectorAll('svg[data-icon-id]');nodes.forEach(function(el){var id=el.getAttribute('data-icon-id');var key='INDEX.'+id;var rep=ci[key];if(!rep)return;var wrap=document.createElement('div');wrap.innerHTML=rep.trim();var fresh=wrap.querySelector('svg');if(!fresh)return;if(!fresh.getAttribute('data-icon-id'))fresh.setAttribute('data-icon-id',id);el.replaceWith(fresh);});}
